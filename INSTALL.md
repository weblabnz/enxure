...
enXure runs with zero configuration — `docker compose up -d --build` works as-is, no `.env` file needed. Skip straight to [step 2](#2-email--smtp-setup) if that's all you want for now; come back here later to customize anything.
...
enXure sends invoice emails via any standard SMTP account. If you don't have one to use, the easiest option is a Gmail account with an **app password**:
...
4. Create a new app password — name it something like "enXure".
...
If you'd rather verify invoice/reminder emails without risking a real send — or don't have SMTP credentials yet — point enXure at a local mail-sink instead of a real provider. [Mailpit](https://mailpit.axllent.org/) is a lightweight SMTP server that captures everything sent to it in a web inbox instead of actually delivering it; nothing ever leaves your machine.
...
Then in `.env`, point enXure's SMTP settings at it instead of a real provider:
...
This starts four containers: `enxure-db` (MySQL), `enxure-php`, `enxure-nginx`, and `enxure-cron`. Check they're all healthy:
...
Without a license, enXure is browsable but won't send real invoices or run recurring billing. Go to the **License** tab in the sidebar, paste in the key you were given, and click
...
Go to **Settings** to set your business name, logo, brand color, footer text (bank details/payment instructions), currency, and tax-year start month. These only affect what your clients see on invoices — the app's own interface always identifies itself as enXure.
...
Your data isn't affected by rebuilding the images: clients/invoices/settings live in a Docker named volume for the database, and generated invoice files/backups live in the `enxure-invoices/` and `enxure-backups/` folders next to this file (ordinary host folders, not Docker-managed volumes — you can back them up directly with normal tools).

## Backups

Use the **Data Management** tab in the app to generate and restore database backups. Backup files land in the `enxure-backups/` folder next to this file — not reachable from outside the app over HTTP, but directly browsable/backupable on the host.

## Migrating to a new server

enXure has exactly one admin account and no phone-home license server, so moving to a new machine is just: back up, copy the file, restore.

On the **old** server, in **Data Management**:

1. Under "Select Tables to Export", leave the default `enxure_*` tables checked and click **Create Backup**.
...
enXure has exactly one admin account and no self-service "forgot password" email flow (there's no guarantee SMTP is even configured). If you're locked out, reset it from the command line on the machine running enXure:

```bash
cd docker-enxure
docker compose exec db mysql -u enxure -p enxure -e "DELETE FROM enxure_users;"
# password is DB_PASSWORD from your .env, or enxure_default_change_me if unset
```
...
```bash
cd docker-enxure
docker compose exec db mysql -u enxure -p enxure -e "UPDATE enxure_users SET totp_secret = NULL, totp_secret_pending = NULL;"
```
