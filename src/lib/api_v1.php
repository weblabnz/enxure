...
if (isset($_GET['apiv1'])) {
    ...
    $apiToken = enxureAuthenticateApiRequest($mysqli);
    if (!$apiToken) {
...
if (!$isAuth && !$isCron) {
    ...
    echo '<script>const docTitles={readme:"Quick Start",install:"Installation Guide"};function loadDoc(doc,anchor){document.getElementById("docModalTitle").textContent=docTitles[doc]||"Documentation";var body=document.getElementById("docModalBody");body.innerHTML=\'<p style="color:#90a0bb;">Loading…</p>\';fetch("?doc="+doc).then(function(r){return r.text()}).then(function(html){body.innerHTML=html;if(anchor){var el=body.querySelector("#"+CSS.escape(anchor));if(el)el.scrollIntoView({block:"start"})}}).catch(function(){body.innerHTML=\'<p style="color:#90a0bb;">Failed to load document.</p>\'})}function openDocModal(doc){loadDoc(doc);document.getElementById("docModal").classList.add("active")}function closeDocModal(){document.getElementById("docModal").classList.remove("active")}function enxureGoToDoc(target,anchor){loadDoc(target,anchor);document.getElementById("docModal").classList.add("active")}document.addEventListener("click",function(e){if(e.target.id==="docModal")closeDocModal()});</script>';
    echo '</div></body></html>';
    exit;
}
